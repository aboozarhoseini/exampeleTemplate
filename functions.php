...
// like functions
function testTheme_set_like_post_meta($id)
{
    if (intval($id)) {
...
function testTheme_get_like_post_meta($id)
{
    if (intval($id)) {
        $current_post_like = get_post_meta($id, 'like', true);
        return intval($current_post_like);
    }
    return 0;
}
